...
验证 main 流程里的公文预创建：识别 → merge → 打印 → 返回文种名。
不真调 officecli.exe。
"""
...
class TestPrepareOfficialDoc:
    """_prepare_official_doc 编排（mock merge）。"""

    def test_calls_merge_and_returns_type(self, monkeypatch, tmp_path, capsys):
        """正常情况：调 merge + 返回文种名。"""
        merge_calls = []
    ...
    def test_missing_template_returns_none(self, monkeypatch, tmp_path):
        """模板缺失时返回 None。"""
    ...
    def test_merge_failure_returns_none(self, monkeypatch, tmp_path):
        """merge 失败时返回 None（不抛）。"""
    ...
    def test_merge_data_has_required_keys(self, monkeypatch, tmp_path):
    ...
    def test_upward_doc_has_signer_in_data(self, monkeypatch, tmp_path):
